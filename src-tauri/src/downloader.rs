...
impl Aria2Client {
    ...
    async fn call(&self, method: &str, params: Vec<serde_json::Value>) -> Result<serde_json::Value> {
        ...
        if let Some(error) = body.error {
            return Err(anyhow::anyhow!("aria2 错误: {} ({})", error.message, error.code));
        }
        
        body.result.ok_or_else(|| anyhow::anyhow!("aria2 返回空结果"))
    }
    ...
    pub async fn add_magnet(&self, magnet: &str, options: Option<DownloadOptions>) -> Result<String> {
        ...
        if let Some(opts) = options {
            ...
            if let Some(limit) = opts.max_download_limit {
                options_map.insert("max-download-limit".to_string(), serde_json::Value::String(limit));
            }
            if let Some(limit) = opts.max_upload_limit {
                options_map.insert("max-upload-limit".to_string(), serde_json::Value::String(limit));
            }
            params.push(serde_json::Value::Object(options_map));
        }
        
        let result = self.call("aria2.addUri", params).await?;
        let gid = result.as_str()
...
pub fn get_aria2_client() -> &'static Aria2Client {
    ARIA2_CLIENT.get_or_init(|| {
        Aria2Client::new("http://localhost:6800/jsonrpc")
    })
}
