...
#[cfg(target_os = "windows")]
unsafe extern "system" fn enum_windows_callback(hwnd: windows::Win32::Foundation::HWND, lparam: isize) -> i32 {
    ...
    if len > 0 {
        ...
        if title.contains("ChangLi") {
            let found = &mut *(lparam as *mut windows::Win32::Foundation::HWND);
            *found = hwnd;
    ...
    std::thread::spawn(move || {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            unsafe {
                ...
                EnumWindows(Some(enum_windows_callback), &mut found_hwnd as *mut _ as isize);
