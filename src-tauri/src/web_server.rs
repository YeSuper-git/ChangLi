    ...
    if let Some(range_str) = range_header {
        ...
        if let Some(range_val) = range_str.strip_prefix("bytes=") {
            ...
            if parts.len() == 2 {
                ...
                return Ok((StatusCode::PARTIAL_CONTENT, headers, axum::body::Body::from_stream(stream)).into_response());
            }
        }
    }
...
async fn api_actors(
    ...
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let actors: Vec<serde_json::Value> = rows.iter().map(|row| {
        let id: i64 = row.get("id");
...
async fn api_actor_series(
    ...
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let series: Vec<serde_json::Value> = rows.iter().map(|row| {
        serde_json::json!({
            "id": row.get::<i64, _>("id"),
            "title": row.get::<String, _>("title"),
            "description": row.get::<Option<String>, _>("description"),
...
pub async fn start_web_server(db: Arc<Mutex<Option<SqlitePool>>>) {
    ...
    ).ok().and_then(|s| s.trim().parse().ok()).unwrap_or(9527);
    ...
    match TcpListener::bind(&addr).await {
        ...
        Err(e) => {
            eprintln!("[ChangLi Web] ❌ 启动 Web 服务器失败 (端口 {}): {}", port, e);
...
pub fn save_web_server_settings(enabled: bool, port: u16) -> Result<(), String> {
    ...
    std::fs::write(config_dir.join("web_server_enabled"), if enabled { "true" } else { "false" }).map_err(|e| e.to_string())?;
