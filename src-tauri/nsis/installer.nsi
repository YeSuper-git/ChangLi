...
; 1. ChangLi custom branded installer front page
Var ChangLiPathText
Var ChangLiPathBrowse
Var ChangLiHeroBitmap
Var ChangLiHeroBitmapHandle
Var ChangLiRunCheckbox
Var ChangLiRunCheckboxState
...
; 3. Install mode is initialized from config, no default install-mode UI page.
...
Function PageReinstall
  ...
  ; ChangLi additionally checks HKCU and HKLM explicitly so upgrading works even
  ; when a user previously chose a different install mode on the "both" installer.
  ...
  Pop $R0
  ; ChangLi custom behavior:
  ; If the same or an older version is already installed, silently run the
  ; bundled uninstaller in passive mode and continue. This makes new versions
  ; cover old versions without asking the user to uninstall first.
  ${If} $R0 == 0
  ${OrIf} $R0 == 1
    DetailPrint "检测到已安装的长离，正在自动卸载旧版本以继续覆盖安装..."
    ClearErrors
    ...
    ${OrIf} ${FileExists} "$INSTDIR\${MAINBINARYNAME}.exe"
      MessageBox MB_ICONEXCLAMATION "无法自动卸载旧版本。请先关闭长离和正在播放的视频窗口，然后重新运行安装程序。"
      Abort
    ${Else}
      StrCpy $0 $R1 1
...
; 5. Start menu folder uses product name; no default start-menu UI page.
Var AppStartMenuFolder

; 6. Installation page
!insertmacro MUI_PAGE_INSTFILES

; 7. ChangLi custom finish page
Page custom ChangLiFinishPageCreate ChangLiFinishPageLeave
...
Function ChangLiBrandPageCreate
  Call SkipIfPassive
  ...
  SetCtlColors $0 0x18171D 0xFFFFFF

  ; Left branded visual panel. Uses the same generated rose/orange sidebar bitmap.
  ${If} "${SIDEBARIMAGE}" != ""
    ${NSD_CreateBitmap} 0 0 164u 314u ""
    Pop $ChangLiHeroBitmap
    ${NSD_SetImage} $ChangLiHeroBitmap "${SIDEBARIMAGE}" $ChangLiHeroBitmapHandle
  ${EndIf}

  ; Step indicator
  ${NSD_CreateLabel} 188u 16u 52u 10u "●  ○  ○"
  Pop $1
  SetCtlColors $1 0xFB5B7B 0xFFFFFF

  ${NSD_CreateLabel} 188u 34u 240u 16u "ChangLi ${VERSION}"
  Pop $1
  SetCtlColors $1 0xFB5B7B 0xFFFFFF

  ${NSD_CreateLabel} 188u 56u 255u 26u "准备安装长离"
  Pop $1
  SendMessage $HWNDPARENT ${WM_GETFONT} 0 0 $2
  SendMessage $1 ${WM_SETFONT} $2 1
  SetCtlColors $1 0x18171D 0xFFFFFF

  ${NSD_CreateLabel} 188u 84u 282u 22u "安装完成后即可打开你的本地影音资料库，继续保留原有数据与播放环境。"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ; Install location card
  ${NSD_CreateGroupBox} 188u 118u 282u 54u "安装位置"
  Pop $1
  SetCtlColors $1 0x374151 0xFFFFFF

  ${NSD_CreateText} 204u 138u 204u 18u "$INSTDIR"
  Pop $ChangLiPathText

  ${NSD_CreateButton} 414u 137u 42u 20u "更改"
  Pop $ChangLiPathBrowse
  ${NSD_OnClick} $ChangLiPathBrowse ChangLiBrowseInstallDir

  ; Feature cards
  ${NSD_CreateGroupBox} 188u 186u 86u 58u "保留本地数据"
  Pop $1
  SetCtlColors $1 0x18171D 0xFFFFFF
  ${NSD_CreateLabel} 198u 207u 66u 22u "升级安装会沿用现有资料库。"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ${NSD_CreateGroupBox} 286u 186u 86u 58u "播放器就绪"
  Pop $1
  SetCtlColors $1 0x18171D 0xFFFFFF
  ${NSD_CreateLabel} 296u 207u 66u 22u "内置播放环境，安装后直接使用。"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ${NSD_CreateGroupBox} 384u 186u 86u 58u "一键启动"
  Pop $1
  SetCtlColors $1 0x18171D 0xFFFFFF
  ${NSD_CreateLabel} 394u 207u 66u 22u "完成后可立即打开 ChangLi。"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ${NSD_CreateLabel} 188u 262u 190u 12u "准备就绪 · 约 1 分钟完成"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ; Re-label default wizard buttons to match the custom frontend.
...
Function ChangLiBrowseInstallDir
  nsDialogs::SelectFolderDialog "选择 ChangLi 安装位置" "$INSTDIR"
  Pop $0
  ${If} $0 != error
  ${AndIf} $0 != ""
    StrCpy $INSTDIR $0
    ${NSD_SetText} $ChangLiPathText "$INSTDIR"
  ${EndIf}
FunctionEnd

Function ChangLiBrandPageLeave
  ${NSD_GetText} $ChangLiPathText $0
  ${If} $0 != ""
    StrCpy $INSTDIR $0
  ${EndIf}
  ${If} $ChangLiHeroBitmapHandle P<> 0
    ${NSD_FreeImage} $ChangLiHeroBitmapHandle
  ${EndIf}
FunctionEnd


Function ChangLiFinishPageCreate
  Call SkipIfPassive
  ...
  ${NSD_CreateLabel} 188u 32u 240u 16u "ChangLi ${VERSION}"
  Pop $1
  SetCtlColors $1 0xFB5B7B 0xFFFFFF

  ${NSD_CreateLabel} 188u 58u 260u 28u "长离已安装完成"
  Pop $1
  SetCtlColors $1 0x18171D 0xFFFFFF

  ${NSD_CreateLabel} 188u 92u 286u 28u "现在可以打开 ChangLi，继续管理你的本地影音资料库。"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ${NSD_CreateGroupBox} 188u 138u 282u 58u "安装摘要"
  Pop $1
  SetCtlColors $1 0x374151 0xFFFFFF
  ${NSD_CreateLabel} 204u 160u 240u 18u "$INSTDIR"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF

  ${NSD_CreateCheckbox} 188u 218u 220u 18u "完成后立即打开 ChangLi"
  Pop $ChangLiRunCheckbox
  ${NSD_Check} $ChangLiRunCheckbox

  ${NSD_CreateLabel} 188u 252u 252u 18u "本地数据已保留 · 快捷方式已创建 · 播放环境已就绪"
  Pop $1
  SetCtlColors $1 0x6B7280 0xFFFFFF
...
Function ChangLiFinishPageLeave
  ${NSD_GetState} $ChangLiRunCheckbox $ChangLiRunCheckboxState
  ${If} $ChangLiHeroBitmapHandle P<> 0
    ${NSD_FreeImage} $ChangLiHeroBitmapHandle
  ${EndIf}
  ${If} $ChangLiRunCheckboxState == ${BST_CHECKED}
