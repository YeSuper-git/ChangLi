...
const Home: React.FC = () => {
  ...
  // series_id → 最近观看时间（用轻量映射，不加载全量视频数据）
  const seriesLastWatched = useMemo(() => {
    const map = new Map<number, number>();
    if (playHistory.length === 0 || videoSeriesMap.size === 0) return map;
    for (const h of playHistory) {
      const sid = videoSeriesMap.get(h.video_id);
      if (sid) {
        const t = new Date(h.last_played).getTime();
